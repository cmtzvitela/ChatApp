...
export default function SignUp() {
  ...
  const handleSignUp = async () => {
    ...
    try {
      dispatch(setUser(await userSignUp(user)));
    } catch (error) {
                ...
                onChange={function (event) {
                  if (validatePassword(event.target.value) === false) {
                    return setPasswordHelperText(
                      'Password must be 8 characters long and contain a number and a symbol'
                    );
                  }
                ...
                onChange={function (event) {
                  if (password.length === event.target.value.length) {
                    if (password !== event.target.value) {
                      return setConfirmPasswordHelperText("Password instances aren't the same");
                    }
                  }

                  setConfirmPassword(true);
                  return setConfirmPasswordHelperText('');
                }}
              />
              <FormHelperText>{confirmPasswordHelperText}</FormHelperText>
            </FormControl>
            <GeneralPurposeButton buttonText="Submit" onClick={handleSignUp}></GeneralPurposeButton>
          </Stack>
        </Paper>
      </Box>
    </Grid>
