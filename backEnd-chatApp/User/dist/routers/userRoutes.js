...
router.patch('/friendRequests', async (req, res) => {
    try {
        const requested = await User.find({ email: req.body.friendEmail });
...
router.get('/search_user', async (req, res) => {
    try {
        const results = await User.find({ username: req.body.username }, 'username id avatar');
        if (results.length < 1) {
